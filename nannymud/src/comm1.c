    ...
    int a1, a2, a3, a4, a5, a6, a7, a8, a9;
{
	...
	if ((n = write(ip->socket, buff2, chunk)) == -1) {
	    if (errno == EMSGSIZE) {
		fprintf(stderr, "comm1[%d]: write EMSGSIZE.\n", ip->socket );
		return;
	    }
	    if (errno == EINVAL) {
		fprintf(stderr, "comm1[%d]: write EINVAL.\n", ip->socket );
	        if (old_message_length) remove_flush_entry(ip);
		ip->do_close = 1;
		return;
	    }
	    if (errno == ENETUNREACH) {
		fprintf(stderr, "comm1[%d]: write ENETUNREACH.\n", ip->socket);
	        if (old_message_length) remove_flush_entry(ip);
		ip->do_close = 1;
		return;
	    }
	    if (errno == EHOSTUNREACH) {
		fprintf(stderr, "comm1[%d]: write EHOSTUNREACH.\n", ip->socket );
	        if (old_message_length) remove_flush_entry(ip);
		ip->do_close = 1;
		return;
	    }
	    if (errno == EPIPE) {
		fprintf(stderr, "comm1[%d]: write EPIPE detected\n", ip->socket );
	        if (old_message_length) remove_flush_entry(ip);
		ip->do_close = 1;
		return;
	    }
	    if (errno == EWOULDBLOCK) {
		fprintf(stderr, "comm1[%d]: write EWOULDBLOCK. Message discarded.\n", ip->socket );
	        if (old_message_length) remove_flush_entry(ip);
/*		ip->do_close = 1;  -- LA */
		return;
	    }
...
#endif
	if (n != chunk)
	    fprintf(stderr, "write socket[%d]: wrote %d, should be %d.\n",
		    ip->socket, n, chunk);
	continue;
    }
    length -= from;
    ...
    int size;
{
...
#if defined(sun) || defined(M_UNIX) || defined(NeXT)
	    if (ioctl(new_socket, FIONBIO, &tmp) == -1)
	        perror("ioctl socket FIONBIO");
#else /* sun */
	    if (fcntl(new_socket, F_SETFL, FNDELAY) == -1)
	        perror("ioctl socket F_SETFL");
#endif /* sun */
	    new_player(new_socket, &addr, length);
	}
	else if (errno != EWOULDBLOCK && errno != EINTR && errno != EPROTO) {
	    perror("accept");
	    abort();
	}
	...
	for(i=0;i<MAX_EFUN_SOCKS;i++) 
	{
	  if(lpc_socks[i].state != CLOSED) 
	  {
	    ...
	    {
	      FD_SET(lpc_socks[i].fd,&readfds);
	      if(lpc_socks[i].fd >= nfds)
		nfds = lpc_socks[i].fd + 1;
	    }
	    if (lpc_socks[i].flags & S_BLOCKED)
	    {
	      FD_SET(lpc_socks[i].fd,&writefds);
	      if(lpc_socks[i].fd >= nfds)
		nfds = lpc_socks[i].fd + 1;
	    }
	  }
	...
	if (res) { /* waiting packets */
		...
		if (fgets(buf, sizeof buf, f_ip_demon)) {
		  ...
		  if (pp) {
		    *pp++ = '\0';
		    ...
		    if (q) {
		      *q = '\0';
		      if(strncmp(buf, "ip", 2) == 0) {  /* query_ip_name */
			laddr=inet_addr(buf + 3);
			if (laddr != -1) {
			  add_ip_entry(laddr, pp);
			}
		      }
		      else  /* query_ident */
			add_ident_to_player(buf + 3, pp);  
		    }
		  }
		}
	    }
	    ...
	    for(i=0; i<MAX_EFUN_SOCKS; i++) 
	    {
	      if(lpc_socks[i].state != CLOSED)
	      {
		if(FD_ISSET(lpc_socks[i].fd,&readfds))
		  socket_read_select_handler(i);
		/* Socket may not exist any longer */
		if(lpc_socks[i].fd >= 0 && FD_ISSET(lpc_socks[i].fd,&writefds))
    ...
    struct object *ob;
{
    ...
    if(!ob->interactive) return; /* Profezzorn */
...
void ident_get_identifier(player)
     struct interactive *player;
{
  ...
  len = sizeof(sin);
  if(getsockname(player->socket, (struct sockaddr *) &sin, &len) < 0)
    return;

  
  /* Let hname do the lookup */
#ifdef OLD_STUPID_IP_DEMON
  if (f_ip_demon_wr != NULL) 
  {
    fprintf(f_ip_demon_wr, "id %s %u %u %u %u\n", player->ob->name, 
	    sin.sin_addr.s_addr,
	    player->addr.sin_addr.s_addr,
	    sin.sin_port,
	    player->addr.sin_port);
...
#ifdef OLD_STUPID_IP_DEMON
void add_ident_to_player(name, ident)
     char *name;
     char *ident;
{
  int i;
  ...
  for (i=0; i<MAX_PLAYERS; i++) {
    ...
    if(all_players[i] && strcmp(all_players[i]->ob->name, name) == 0)
      all_players[i]->ident = make_shared_string(ident);
  }
}
#endif
...
#ifdef OLD_STUPID_IP_DEMON
        if (f_ip_demon_wr != NULL) {
            fprintf(f_ip_demon_wr, "ip %s\n", query_ip_number(ob));
            fflush(f_ip_demon_wr);
        }
#endif
	logon(ob);
	ident_get_identifier(ob->interactive);
    ...
    char *str;
{
  ...
  free_object(i->input_to->ob, "call_function_interactive");
  function = string_copy(i->input_to->function);
  onerror_num=onerror(my_free,function);
  ob = i->input_to->ob;
...
void remove_all_players()
{
	...
	push_number(1);
	(void)apply("save_me", all_players[i]->ob, 1);
    }
}
     ...
     char *str;
{
  if(!command_giver || !command_giver->interactive ||
     strlen(str) > 79)
    return;
  strcpy(command_giver->interactive->prompt, str);
}
...
void save_prompt()
{
  if(!command_giver || !command_giver->interactive)
    return;  
  strcpy(command_giver->interactive->old_prompt,
    ...
    struct object *ob;
{
  ...
  if (!ob || ob->interactive == 0)
    return 0;
  sprintf(buf, "%d", ob->interactive->addr.sin_port);
  return buf;
}
...
{
    struct interactive *save_interactive;
    char *p;

    if(!command_giver || !command_giver->interactive)
	return;
    save_interactive = command_giver->interactive;
    p = command_giver->interactive->default_err_message;
    if(p)
    {
#ifdef SLIMY_NOTIFY_CODE
        char *m;
	...
	if (m != p)
	    free(m);
#else
	add_message("%s", p);   /* add_message calls shadow_catch_message */
#endif
	free_string(p);
	save_interactive->default_err_message = 0;
    }
    else
    {
	add_message("What?\n");
...
#endif
	    push_object(ob);
	    ret = apply_master_ob("query_mudwho_name", 1);
	    if(!ret || ret->type != T_STRING)
	      sprintf(mudwhoname,"------");
	    else
	      sprintf(mudwhoname,"%s", ret->u.string);
	    strcpy(mudwhoid,ob->living_name);
	    if(mudwhoid && islower(mudwhoid[0]))
	      mudwhoid[0] += 'A' - 'a';
	    rwhocli_userlogin(mudwhoid,mudwhoname,
		ob->interactive->login_time);
	}
    }
}
void sendmudwhologout(ob)
struct object *ob;
{
	if (ob->interactive)
	{
	    strcpy(mudwhoname,ob->living_name);
	    if(mudwhoname && islower(mudwhoname[0]))
	      mudwhoname[0] += 'A' - 'a';
...
dump_file_descriptors()
{
    ...
    for (i = 0; i < MAX_PLAYERS + MAX_EFUN_SOCKS + 20; i++) {
     ...
     char *file;
{
  ...
  tmp = allocate_array(4);
  
  tmp->item[0].u.number = stbuf.st_mode;
  tmp->item[1].u.number = stbuf.st_size;

 ...
 *    3-93 : \rjan Rosenblom (Oros@NannyMud) : Ported to Nannymud.
 */
    ...
    char *name;
{
	...
	default:
	    ...
	    buf = (char *) xalloc(sizeof (long) + len + 1);
    ...
    int fd;
{
	...
	case DATAGRAM:
	    ...
	    apply(lpc_socks[fd].read_callback, lpc_socks[fd].owner_ob, 3);
	    return;
	}
	break;
	...
	lpc_socks[fd].flags |= S_WACCEPT;
	push_number(fd);
	apply(lpc_socks[fd].read_callback, lpc_socks[fd].owner_ob, 1);
	return;
	...
	case MUD:
	    ...
	    debug(8192,("read_socket_handler: apply read callback\n"));
	    apply(lpc_socks[fd].read_callback, lpc_socks[fd].owner_ob, 2);
	    break;
#endif
	case STREAM:
    ...
    push_number(fd);
    apply(lpc_socks[fd].close_callback, lpc_socks[fd].owner_ob, 1);
    ...
    int fd;
{
    ...
    push_number(fd);
    apply(lpc_socks[fd].write_callback, lpc_socks[fd].owner_ob, 1);
}
    ...
    struct object *ob;
{
	...
	assign_svalue_no_free(sv, &const0);
}
...
dump_socket_status() {
  ...
  for(i = 0; i < MAX_EFUN_SOCKS; i++) {
    ...
    add_message("%-21s ", inet_address(&lpc_socks[i].r_addr));
    if(lpc_socks[i].owner_ob)
      add_message("%s",lpc_socks[i].owner_ob->name);

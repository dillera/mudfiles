...
int writeflg;
{
	...
	*cp = '\0';
    ...
    {
	if (writeflg)
	    file2 = acl_checkrights(file, ACL_A+ACL_W, 2, 1);
	else
	    file2 = acl_checkrights(file, ACL_R, 0, 1);
    }
    else
    {
#ifdef COMPAT_MODE
    ...
    for (i2=0;i2<MAXLINE;i2++) {
		...
		indent_error=1;
	      ...
	      str_on=0;
            ...
            /* buff[i++]=MAXLINE; /Fel, tror profezzorn */
	    buff[i++]=0;
            i2=1000;
            break;
          case SP:
    ...
    struct object *exit_ob;
{
    ...
    ED_BUFFER->CurPtr = &command_giver->interactive->ed_buffer->Line0;
    ...
    {
	if (cfg_use_acls)
	    tmppath = acl_checkrights(file_arg, ACL_R, 0, 1);
#ifndef COMPAT_MODE
	else
	    file_arg = check_valid_path(file_arg, command_giver->eff_user,
					"ed_start", 0);
#endif	   
...
static void free_ed_buffer() {
	...
	name = ED_BUFFER->exit_fn;
        free((char *)ED_BUFFER);
	ED_BUFFER = 0;
	set_prompt(command_giver->interactive->old_prompt);
...
void save_ed_buffer()
{
	...
	if (stmp->type == T_STRING) {
	    if (fname = acl_checkrights(stmp->u.string, ACL_A+ACL_W, 2, 1))
		dowrite(1, P_LASTLN, fname, 0);
	    else
	    {
		add_message("Can't open \"%s\" for writing!\n",
			    stmp->u.string);
		free_svalue(stmp);
		return;
	    }

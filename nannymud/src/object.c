...
/* #define OLD_SAVE_OBJECT */
#define MAX_SAVE_OBJECT_SIZE 3000000
...
static int save_map_size PROT((vector *v));
static char *save_mapping PROT((vector *v));
    ...
    char *file;
{
	...
	if (strncmp(current_object->name, "obj/", 4) != 0 &&
	    ...
	    strncmp(current_object->name, "local/", 6) != 0) {
...
#else /* OLD_SAVE_OBJECT */


#define SAVE_BEGIN_SIZE 1000
...
void my_putchar(char b)
{
  ...
  if(buffered>=bufsize)
  {
    ...
    if(!buf)
    {
      fatal("Fatal error in putchar\n");
    }
    tmp=(char *)realloc(buf,bufsize);
    if(!tmp)
    {
      tmp=xalloc(bufsize);
      memcpy(tmp, buf, buffered);
      free(buf);
    }
    buf=tmp;
  }
...
void my_strcat(char *b)
{
  int l;
  l=strlen(b);
  while(buffered+l>=bufsize)
  {
    bufsize*=2;
    buf=realloc(buf,bufsize);
  }
  strcat(buf+buffered-1,b);
  buffered+=l;
}

void init_buf()
{
  if(!buf) buf=(char *)xalloc(bufsize=SAVE_BEGIN_SIZE);
  *buf=0;
  buffered=1;
}

int my_get_buf_size() {  return buffered; }
...
static int save_object_error=0;
...
void save_svalue(struct svalue *s)
{
  ...
  if(buffered>MAX_SAVE_OBJECT_SIZE)
  {
    save_object_error=1;
    return;
  }
  ...
  case T_POINTER:
    my_strcat("({");
    for(e=0;e<s->u.vec->size;e++)
    {
      save_svalue(&(s->u.vec->item[e]));
      my_putchar(',');
    }
    my_strcat("})");
    break;
  ...
  case T_MAPPING:
    my_strcat("([");
    for(e=0;e<s->u.vec->item[0].u.vec->size;e++)
    {
      save_svalue(&(s->u.vec->item[0].u.vec->item[e]));
      my_putchar(':');
      save_svalue(&(s->u.vec->item[1].u.vec->item[e]));
      my_putchar(',');
    }
    my_strcat("])");
    break;
...
void save_object(struct object *ob,char * file)
{
  ...
  if (cfg_use_acls)
  {
      file = acl_checkrights(name, ACL_W+ACL_A, 2, 0);
      if (file == 0)
      {
	  free(name);
	  error("Illegal use of save_object()\n");
  ...
  save_object_error=0;
  init_buf();
  for (i=0; i < ob->prog->num_variables && !failed; i++)
  {
    struct svalue *v = &ob->variables[i];

    if (ob->prog->variable_names[i].type & TYPE_MOD_STATIC)
      continue;
    if(v->type==T_OBJECT) continue;

    init_buf();
    if(v->type == T_STRING)
    {
      my_strcat(v->u.string);
      replace_newline(buf);
      if (fprintf(f, "%s \"%s\"\n", ob->prog->variable_names[i].name,
		  buf) == EOF)
	failed = 1;
      ...
      save_svalue(v);
      if(save_object_error)
      {
	failed=1;
	continue;
      }
      if(fprintf(f,"%s %s\n",ob->prog->variable_names[i].name,buf)==EOF)
	failed=1;
    }
  }
  free(free_buf());
  ...
  if(failed)
  {
    ...
    unlink(tmp_name);
    free(name);
    error("save_object failed (disk full?)\n");
  }
    ...
    char *file;
{
    ...
    if (cfg_use_acls)
    {
	file = acl_checkrights(name, ACL_R, 0, 0);
	if (file == NULL)
	{
	    free(name);
	    return 0;
...
#endif
#ifdef COMPAT_MODE
/*Added by Qqqq 920208*/
	if (( strncmp(file,"mail/",5) == 0 ||
	     strncmp(file,"bboard/",7) == 0 ||
	     strncmp(file,"room/post_dir/",14) == 0) &&
	    strncmp(current_object->name, "obj/", 4) != 0 &&
	    strncmp(current_object->name, "room/", 5) != 0 &&
	    strncmp(current_object->name, "std/", 4) != 0 &&
	    strncmp(current_object->name, "local/", 6) != 0)
	{
...
*/
	if (val[0] == '(') {
	    ...
	    if (!v->u.vec) {
	       ...
	       *v = const0;
	       (void)fclose(f);
	       error("Illegal %s format when restoring '%s' from '%s'.\n",
		     vtype == T_MAPPING ? "mapping" : "array", var, name);
	    }
	    continue;
	}
...
void tell_object_low(struct object *ob, char *str)
{
   struct object *save_command_giver = command_giver;
   if(!ob || (ob->flags & O_DESTRUCTED))
      return;   /*  Hmm, lets ignore these for now...  */
   if(ob->interactive)
   {
      if(strlen(str) > 9500)  /* Not pretty */
	 error("Too long string.\n");
      command_giver = ob;
      add_message("%s", str);
   }
   else   /*  if(obj->flags & O_ENABLE_COMMANDS)   */
   {
      push_constant_string(str);
      apply("catch_tell", ob, 1);
   }
    ...
    char *str;
{
   ...
   if(ob->interactive)
   {
     struct object *save_command_giver = command_giver;
     if(strlen(str)>9500)
       error("Too long string to tell_object()\n");
     command_giver = ob;
     add_message("%s", str);
     command_giver = save_command_giver;
   }
    ...
    char *from;
{
   ...
   if(ob->ref == 32766)
   {
      printf("---> Argh! %s: High ref count [%d]\n",
	     ob->name ? ob->name : "<noname>", ob->ref);
   }
#endif
    ob->ref++;
    if (d_flag > 1)
...
void remove_all_objects() {
  ...
  while(1) {
    ...
    if (ob == obj_list)
    {
       fprintf(stderr, "Failed to sdestruct %s\n", ob->name);
       break;
    }
  }
    ...
    int free_sub_strings;
{
    ...
    byte_code_bytes -= progp->program_size;
    function_bytes -= progp->num_functions * sizeof(struct function);
    strings_bytes -= progp->num_strings * sizeof(char *);
    variables_bytes -= progp->num_variables * sizeof(struct variable);
    line_number_bytes -= (char *)progp->functions -
       (char *)progp->line_numbers;
    inherits_bytes -= progp->num_inherited * sizeof(struct inherit);
#endif
    ...
    int arg;
{
    ...
    if(!arg)
    {
      apply("__INIT", ob, 0);
      if(ob->flags & O_DESTRUCTED)
	error("Object destructed in __INIT()\n");
    }
    push_number(arg);
...
{
    if (!ob || !ob->shadowed ||
	(command_giver && command_giver->interactive &&
...
static int is_return(unsigned char *p)
{
  switch(EXTRACT_UCHAR(p))
  {
  case F_RETURN_ZERO - F_OFFSET:
  case F_RETURN_ONE - F_OFFSET:
    return 1;

  case F_CONST1 - F_OFFSET:
  case F_CONST0 - F_OFFSET:
    if(p[1]==F_RETURN-F_OFFSET)
      return 1;
...
static char *skip(char *pc,int args)
{
  short tmp;
  while(1)
  {
    ...
    {
    case F_LOCAL_NAME-F_OFFSET:
      if(args<1) return pc;
      if(pc[1] != 0) return pc;
      if(EXTRACT_UCHAR((pc+2)) == F_JUMP_WHEN_ZERO - F_OFFSET)
      {
	pc+=5;
	continue;
      }

      if(EXTRACT_UCHAR((pc+2)) != F_JUMP_WHEN_NON_ZERO - F_OFFSET)
	return pc;

      pc+=2;
      /* Fall through */

      case F_JUMP-F_OFFSET:
      pc++;
      ((char *)&tmp)[0]=pc[0];
      ((char *)&tmp)[1]=pc[1];
      pc+=tmp;
...
void try_throw_program_away(struct object *o)
{
  struct program *p,*target;
  char *fname;
  int e,calls_reset;

  if(o->flags & O_PROGRAM_NOT_THROWN) return;
  o->flags|=O_PROGRAM_NOT_THROWN;

  p=o->prog;
  if(p->num_inherited != 1) return;
  if(p->inherit[0].variable_index_offset != 0) return;
  target=p->inherit[0].prog;
  ...
  for(e = 0; e < p->num_functions; e++)
  {
    ...
    if(p->functions[e].flags & NAME_INHERITED) continue;
    name=p->functions[e].name;
    if(!strcmp(name,"__INIT")) continue;
    if(!strcmp(name,"reset"))
    {
      funp=p->program+p->functions[e].offset;

      funp=skip(funp, p->functions[e].num_arg);

      if(funp[0]==F_LOCAL_NAME-F_OFFSET &&
	 funp[1]==0 &&                      /* $0 */
	 p->functions[e].num_arg>0 &&       /* local or argument? */
	 funp[2]==F_CALL_FUNCTION_BY_ADDRESS-F_OFFSET &&
	 funp[5]==1 && /* args */
	 funp[6]==F_POP_VALUE-F_OFFSET)
      {
	short tmp;
	((char *)&tmp)[0] = funp[3];
	((char *)&tmp)[1] = funp[4];
	if(!strcmp(p->functions[tmp].name,"::reset"))
	{
	  calls_reset=1;
	  funp+=7;
	}
      ...
      /* if(!_isclone(this_object())) return; */
      if(funp[0]==F_THIS_OBJECT-F_OFFSET &&
	 funp[1]==F__ISCLONE-F_OFFSET &&
	 (funp[2]==F_JUMP_WHEN_ZERO ||
	  funp[2]==F_JUMP_WHEN_NON_ZERO) &&
	 is_return(funp+5))
      {
	short tmp;
	funp+=3;
	...
	funp=skip(funp, p->functions[e].num_arg);
      ...
      if(is_return(funp)) continue;

    }

    /* it might still be a null function
     * but such tests are too complicated at this time
     */

    return;
  }

  /* If reset exists in an inherited object, we can't just ignore it */
  if((fname = function_exists("reset", o)) != NULL)
  {
     char *oname, *ptr;
     oname = xalloc(strlen(o->name) + 3);
     strcpy(oname, o->name);
     for(ptr = oname + strlen(oname) - 1; ptr > oname && isdigit(*ptr); ptr--)
        ;
     if(ptr > oname && *ptr == '#' && *(ptr+1))
        *ptr = '\0';
     strcat(oname, ".c");
     if(strcmp(fname, oname))
        calls_reset = 1;
     free(oname);
  }

  /* Ok, we have a winner */
  for(e=p->num_variables-1;e>=(int)target->num_variables;e--)
    free_svalue(o->variables+e);

#if 0
  fprintf(stderr,"Throwing program for %s (%d bytes).\n",
	 o->name,
	 p->ref==1?p->total_size:0);
#endif
  bytes_thrown+=p->ref==1?p->total_size:0;

  reference_prog(target, "throw program away");
  free_prog(p, 1);
  o->prog=target;

  if(!calls_reset) o->flags|=O_NEVER_RESET;
  o->flags|=O_PROGRAM_THROWN;

  o->flags&=~O_PROGRAM_NOT_THROWN;
  try_throw_program_away(o);
}


void try_throw_away_new_programs()
{
  struct object *o;
  for(o=obj_list;o && !(o->flags & O_PROGRAM_NOT_THROWN);o=o->next_all)
  {
    try_throw_program_away(o);
  }
}

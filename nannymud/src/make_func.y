...
#define FUNC_SPEC 	"make -s list_funcs"
...
%token VOID INT STRING OBJECT MIXED UNKNOWN MAPPING

%token DEFAULT

%type <number> type VOID INT STRING OBJECT MIXED UNKNOWN MAPPING arg_list basic typel
    ...
    char **argv;
{
    ...
    while (i = read(fdr, buffer, COPYBUFSIZ))
       write(fdw, buffer, i);
    close(fdr);
    for (i = 0; i < num_buff; i++) {
       if (has_token[i]) {
...
int yylex1() {
	...
	case '#':
	{
#if defined(sun) && !defined(__svr4__) /* no prototype in <stdio.h> *sigh* */

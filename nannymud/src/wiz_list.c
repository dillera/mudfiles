...
void wiz_decay() {
	...
	wl->call_outs = wl->call_outs * 9 / 10;
    }
    ...
    char *v;
{
	...
	if (!all && num > 15 && (num < this_pos - 2 || num > this_pos + 2))
	{
	  ...
	  add_message("%-15s %5d %2d%% (%d)\t[%5dk,%5d,%5d] %7d %d\n", wl->name,
...
struct vector *get_wiz_list() 
{
    struct vector    *v;
    struct wiz_list  *wl;
    int               i;

    for (i = 0, wl = all_wiz; wl; i++, wl = wl->next)
      /* Do nothing */ ;
    v = allocate_array(i);
	...
	tmp = allocate_array(7);
	
	tmp->item[0].type = T_STRING;
	tmp->item[0].string_type = STRING_SHARED;
	tmp->item[0].u.string = make_shared_string(wl->name);
	tmp->item[1].u.number = wl->score;
	tmp->item[2].u.number = wl->cost;
	tmp->item[3].u.number = wl->heart_beats;
	tmp->item[4].u.number = wl->total_worth;
	tmp->item[5].u.number = wl->size_array;
	tmp->item[6].u.number = wl->call_outs;

	v->item[i].type = T_POINTER;
	v->item[i].u.vec = tmp;		/* Ref count is already 1 */
    }
    return v;
  }

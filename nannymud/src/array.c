...
/*#if defined(__GNUC__) && !defined(lint) && !defined(sparc)
...
struct vector null_vector = {
    ...
    &null_vector,     /* previous pointer */
    &null_vector,     /* next pointer */
#endif
    0,	              /* size */
    VECTOR_CLEAN,     /* flags */
...
#ifdef SIMPLE_DEBUG
static int zero_vec_refs=0;
#endif

#ifndef NO_ARRAY_LINKS
struct vector *vector_list = &null_vector;
#endif

/*
 * Allocate an array of size 'n'.
 */
struct vector *allocate_array_no_init(int n)
{
    struct vector *p;
...
#ifndef NO_ARRAY_LINKS
    p->prev_all = vector_list;
    vector_list->next_all = p;
    p->next_all = vector_list->next_all;
    vector_list->next_all->prev_all = p;
#endif
...
struct vector *allocate_array(int n)
{
    ...
    p=allocate_array_no_init(n);
    for (i=0; i<n; i++)
	p->item[i] = const0;
    return p;
}

static void subtract_array_count(struct vector *p)
{
  if (p->user)
    p->user->size_array -= p->size;
  total_array_size -= sizeof (struct svalue) * p->size;
}
...
void free_vector(struct vector *p)
{
  int i;
#ifdef SIMPLE_DEBUG
  if(zero_vec_refs > null_vector.ref)
    fatal("Zero vector references lowered outside of free_vector!\n");
#endif    

  p->ref--;
#ifdef SIMPLE_DEBUG
  zero_vec_refs=null_vector.ref;
#endif
  if (p->ref > 0)
    return;
#ifdef DEBUG
    ...
    int n;
{
    if (n <= p->size >> 1)
    {
      ...
      res=allocate_array_no_init(n);
      res->flags=p->flags;
      memcpy((char *)(res->item),(char *)(p->item),sizeof(struct svalue)*n);
      subtract_array_count(p);
      p->size=0;
      free_vector(p);
      return res;
...
/*    if (strlen(str) < len || strcmp(str + strlen(str) - len, del) != 0)
    ...
    ret = allocate_array_no_init(num);
    ...
    if (*beg != '\0') {
      /* free_svalue(&ret->item[num]); Not needed here either */
	ret->item[num].type = T_STRING;
	ret->item[num].string_type = STRING_MALLOC;
...
#ifdef HYPER_INVIS
      push_object(ob);
      r=apply_master_ob("is_hard_invis",1);
      if (r == 0 || r->type != T_NUMBER || r->u.number==0)
#endif
      {
    ...
    int to;
{
    ...
    d = allocate_array_no_init(to-from+1);
    ...
    struct svalue *arg2;
{
	...
	return arg1->u.number == arg2->u.number;
      ...
      if (arg1->string_type == STRING_SHARED &&
	  arg2->string_type == STRING_SHARED)
	return arg1->u.string == arg2->u.string;
      else
	return strcmp(arg1->u.string, arg2->u.string) == 0;
    ...
    struct vector *p, *r;
{
    ...
    d = allocate_array_no_init(p->size+r->size);
    res=0;
    for (cnt=0;cnt<p->size;cnt++) {
    ...
    d->flags = p->flags & r->flags;
    return d;
}

struct vector *add_array_destructively(struct vector *p, struct vector *r)
{
    int cnt,res;
    struct vector *d;
    ...
    if(p->ref == 1)
    {
      memcpy((char *) d->item, (char *)p->item, sizeof(struct svalue) * p->size);
      res+=p->size;
      subtract_array_count(p);
      p->size=0;
    }else{
    ...
    if(r->ref == 1)
    {
      memcpy((char *) (d->item + res), (char *)r->item, sizeof(struct svalue) * r->size);
      subtract_array_count(r);
      r->size=0;
    }else{
      for (cnt=0;cnt<r->size;cnt++) {
	assign_svalue_no_free (&d->item[res],&r->item[cnt]);
    ...
    struct svalue *extra;
{
  ...
  extern int remove_onerror();
    ...
    onerror_num=onerror(free_vector, r); /* Profezzorn Mars 29 1995 */
    ...
    struct object *ob;
{
  ...
  onerror_num=onerror(free_vector, inlist);
    keynum = inlist->size;
    root = inlist->item;
...
static int deep_inventory_size(struct object *o)
{
  int num;
  num=0;
  for(o=o->contains;o;o=o->next_inv)
    num+=deep_inventory_size(o)+1;
  return num;
}

struct vector *deep_inventory(struct object *ob)
{
  int size, ptr, ptr2;
  struct vector *inv;

  size=deep_inventory_size(ob);
  inv=allocate_array_no_init(size);
  ...
  while(1)
  {
    ...
    {
      inv->item[ptr].type=T_OBJECT;
      inv->item[ptr].u.ob=ob;
      add_ref(ob, "deep_inventory");
      ptr++;
    }
    if(ptr2==size) break;
    ob=inv->item[ptr2++].u.ob;
  }
  return inv;
}
...
struct vector *order_alist(struct vector *inlist)
{
    struct vector *outlist;
...
struct vector *get_spec_obj(int mask, int choice)
{
   ...
   int number;
   choice &= mask;
   for(number = 0, obj = obj_list; obj; obj = obj->next_all)
   {
      if(!(obj->flags & O_DESTRUCTED) && (obj->flags & mask) == choice)
         number++;
   }
   spec_obj = allocate_array_no_init(number);
   if(number > 0)
   {

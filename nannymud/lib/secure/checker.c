...
private static nomask object old, target;
...
nomask mixed query_path()
{
  if(this_player() == target)
  {
    return (string) target->query_path();
  }
  return ({ "En helt normal path" });
}

nomask status query_invis()
{
   if(!this_player(1) || (int) this_player(1)->query_level() < 24)
      return 1;
   else
      return 0;
}

 ...
 *  Log of changes
 * ------------------------------------------------------------
 *  Date    By whom  Did what
 *  940731  Brom     Adjusted hp/level as to have the same as /obj/monster.c.
 *  940805  Brom     More possible replacement strings in attack chats.
 *  950318  Taren    Made the give_money_handler work
 *  950709  Qqqq     Removed strict types to fix bug with wrong number of args
 *                   with runaway.
 */

#define SMARTMONSTER_VERSION	"0.6.3 (Oct 11, 1993)"
...
string substitute_other(string str, string opponents_name) {
    string He, Him, His, he, him, his;
    object enemy;

    /* First, the old code revisited. */
    str = replace(str, "$OTHER", opponents_name);
    str = replace(str, "$LOWOTHER", lower_case(opponents_name));

    /* Then, the new code */
    if (!objectp(this_object())) return "";
    if (!environment(this_object())) return "";
    enemy = present(opponents_name, environment(this_object()));
    if (enemy)
      {
	he  = (string)(enemy->query_pronoun());
	him = (string)(enemy->query_objective());
	his = (string)(enemy->query_possessive());
	He  = capitalize(he);
	Him = capitalize(him);
	His = capitalize(his);
      }
    ...
    str = replace(str, "$HE", He);
    str = replace(str, "$HIM", Him);
    str = replace(str, "$HIS", His);
    str = replace(str, "$LOWHE", he);
    str = replace(str, "$LOWHIM", him);
    str = replace(str, "$LOWHIS", his);
		      ...
		      mixed response, string opponent_msg)
{
	...
	if (opponent_obj && objectp(opponent_obj)) {
...
status do_matching(string str) {
    ...
    /* Rewritten by Banshee 950218 to match the new given-message. */
    if (   give_money_handler_obj
	&& (sscanf(str, "%s gives coins to %s.\n", who, whom) == 2
	    || sscanf(str, "%s gives coin to %s.\n",who, whom)==2)
	&& ( id(whom) 
	    || id(lower_case(whom))
	...
	who_obj = smartpresent(who, environment(this_object()));
	if (!who_obj) {
	    SAY_CONFUSED(who + " gave me some money, but doesn't seem to be here now.");
	}
	else
...
void do_the_give_magic(object o)
{
  object tp;
  tp=this_player();
  enable_commands();
  if(intp(o) && give_money_handler_obj)
    call_other(give_money_handler_obj, give_money_handler_fun, tp, tp->query_name(),o);
  
  if (give_handler_obj && objectp(o))
    /* Ok, a known someone gave this monster something! Now: what? */
    call_other(give_handler_obj, give_handler_fun, tp,
	       tp->query_name(), o, tp->_name(o));
}
...
void _signal_gotten(object o)
{
  object tp;
  if(o->query_money())
      return call_out("do_the_give_magic", 1, o->query_value());
  call_out("do_the_give_magic", 1, o);
}
...
void extra_attacks() {
  ...
  if (attack_data == 0)
    return;
  /* Do not perform this if the opponent has a block_attack_hook. Really, we would
   * like to test if the function block_attack_hook() in the hooking object returns 1.
   * This object is not available, however. Thus we don't do spells if there is such
   * a hook at all.
   * Patch by Brom 951024.
   * 
   * Why? Block attack hook is ment to be checked in the monster itself
   * not in the attacker_ob.
   * Changed by Qqqq, 960131.
   */
  if (block_hook("block_attack_hook",({attacker_ob,0})))
    return 0;
...
void random_move()
{
    ...
    if (objectp(this_object()) && environment(this_object()) == old_env) {
...
heart_beat()
{
...
#ifdef USE_WETCODE
    if (wet_time > 0 && (environment(this_object())) &&
    ...
    if (!test_if_any_here() && waiting_texts) {
	/*  No players (real, interactive ones, that is!) in this room,
	 *  but there are messages in the queue.
	 *  Pehaps we are supposed to follow a player that just left?
	 */
	match_the_queue();
    }

    if (!test_if_any_here()) {
	/*  Still no players in this room!
	 *  Ok, there might have arrived new texts, waiting to be processed,
	 *  but we don't process them as long as the room is player-empty.
	 *  (We used to, and Harry and Sir Obliterator stood alone in a room
	 *  and talked for ever. Yuk.)
	 */
...
/* Those functions have been slightly changed in monster.c
   Brom 940731
...
/* The new monster has a rather different set_up of hp, ac etc.
   It also checks for lvl<0. Brom 940731
*/
...
void long() {
    ...
    cap_pronoun=capitalize(this_object()->query_pronoun());
...
status query_is_smartmonster() { return 1; }

/*---------------------------------------------------------------------------*/

/* Patch by profezzorn */
int hit_player(int damage,int flag)
{
  int i;
  i= ::hit_player(damage,flag);

  /* I don't trust is_friend */
  if(friends && sizeof(friends) && attacker_ob)
  {
    if(member_array(attacker_ob, friends) != -1)
    {
      stop_fight();
    }
    else if(!interactive(attacker_ob) &&
	    member_array(attacker_ob->query_real_name(), friends) != -1)
    {
      stop_fight();
    }
  }

...
string messages="";
int num_messages; /* Why not save this one? /Milamber */
int max_num_messages=30;
string name;
int write_level;
int read_level;
...
new(hd)
{
  ...
  if (!this_player() ||
      ...
      this_player()->query_real_name()=="guest")
  {
...
static get_body(str)
{
  ...
  }else if (this_player()->query_level() >= 22 && 
...
string display_note(int i)
{
  ...
  while(rest != "")
  {
    ...
    if (i == 0) {
      mixed res;
      res=({ "Foo","Nobody","BlaBla",0 });
      sscanf(hd,"%s\n(nailed by %s)",hd,by);
      sscanf(hd,"%s (%s, %*s)",res[0],res[1]);
      res[2]=body;
      res[3]=by && ({ by, 23 });
      return res;
    }
...
static remove(str)
{
  ...
  while(rest != 0 && rest != "")
  {
    ...
    if(sscanf(hd,"%s\n(nailed by %s)\n",hd,by))
    {
      if(this_player(1)->query_level()<=22)
      {
...
static nail(str)
{
  ...
  while(rest != 0 && rest != "")
  {
	...
	return 1;
	  ...
	  " produces a hammer and secures a note\ntitled '" + hd + "' to the board with some nails.\n");

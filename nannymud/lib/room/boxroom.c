 ...
 *  NOTE: To work in some driver versions, this room has to be in /room or /obj!
 ...
 *	This file latest updated March 4, 1993 by Padrone.
 *
 *      Check for property 'no_deposit' added June 19, 1994 by Slater.
 *
 *      Upgraded to use simul_efun get_objects() July 8, 1994 by Slater.
 *      Define OLD_STORE_CODE to use the old code.
 *
 *      Where the F*** did my mapping optimizations go??????
 *               /Profezzorn
 */
...
#include "/players/padrone/padconfig.h"
#include "/players/padrone/smartroom.h"

SIMPLE_ROOM( ({DEPOSIT, FROM_BOXROOM }),
...
reset(arg)
{
  ...
  ::reset(arg);
	...
	if (safe_guard != "Everything is OK.") {
      ...
      write_file("log/SECURE/PADRONE_STORE", ctime(time()) + "\n");
    }
}
...
/*  Check if "client_name" is a legal name
 *  -- otherwise it will crash the client list!
 *  Return 1 or 0.
 */
bad_name(client_name) {
    string junk1, junk2;
    ...
    if (client_name == "some mist" || client_name == "Some mist") {
	write("You are dead. Go away.\n");
        return 1;
    }
    if (sscanf(client_name, "%s %s", junk1, junk2) == 2) {
	write("Your name contains a space. Go away.\n");
	return 1;
    }
    return 0;
} /* bad_name */
...
/*  Function to check for some properties and such.
 *  With an uptime of days, lawbringer etc. should perhaps not 
 *  be stored.
 *  Brom 951007
 */

do_check(item, verbose)
{
  object inv, thing;
  int i;

  if (item -> query_property("no_deposit") || 
      item -> query_property("special_item") ||
      item -> drop())
    return 0; /* NOT OK! */
  if (item) inv = deep_inventory(item); /* Better safe than sorry. */
  
  for (i=0; i < sizeof(inv); i++)
  {
    thing = inv[i];
    if (thing -> query_property("special_item") ||
	thing -> query_property("no_deposit"))
    {
      if (verbose && thing -> short())
	write("You are not allowed to store " +
	      thing -> short() + ", others might need that!\n");
      return 0; /* NOT OK! */
    }
    if (thing -> query_money()) { /* Banshee 960307 */
      write("You are not allowed to store money.\n");
      return 0; } /* NOT OK! */
  }
  return 1; /* This is OK to store. */
} /* do_check */
 ...
 * 
 *  Check for property 'no_deposit' was added by Slater, 940618
 */
store_one_item(the_item, the_box, verbose) 
{
  ...
  if (!do_check(the_item, verbose))
  {
    if (verbose)
      write("I can't take it from you!\n");
    return 0;
  }
  if(the_item -> query_money()) /* Profezzorn */
  {
    if(verbose)
      write("You can't store money.\n");
    return 0;
  }

  client_name = this_player()->query_name();
  weight = the_item->query_weight();
  environment(the_item)->add_weight(-weight);

  item_short = the_item->short();
  write_file("log/SECURE/PADRONE_STORE",
	   client_name+": "+file_name(the_item) + "\n");
  ...
  if (value > INSURANCE_LIMIT)
    ...
    write("Were you aware that " + item_short + " is worth " + value + " coins?\n");
    log_file(LOGFILE, ctime(time()) + ": store_one_item: " + client_name + " '" +
	     item_short + "', " + file_name(the_item) + ", creator = " +
	     creator(the_item) + ", value = " + value + "\n");
    value = 0;
  }
  return value;
}				/* store_one_item */
...
store_item(item_string)
{
...
#else
  {
    object *objects;
    int i;
    objects = get_dropable_objects(item_string, ({this_player()}));
    i = sizeof(objects);
    while(i--)
      adjustment += store_one_item(objects[i], the_box, 1);
  }
#endif
  adjust_list(client_name, adjustment);
  save_this_room();
...
retrieve_one_item(the_item, the_box, verbose) {
	...
	write("Were you aware that " + item_short + " is worth " + value + " coins?\n");
	log_file(LOGFILE, ctime(time()) + ": retrieve_one_item: " + client_name + " '" +
		 item_short + "', " + file_name(the_item) + ", creator = " +
		 creator(the_item) + ", value = " + value + "\n");
	value = 0;
    }
...
retrieve_item(item_string)
{
...
#else
  {
    object *objects;
    int i;
    objects = get_objects(item_string, ({the_box}));
    i = sizeof(objects);
    while(i--)
      neg_adjustment += retrieve_one_item(objects[i], the_box, 1);
  }
#endif

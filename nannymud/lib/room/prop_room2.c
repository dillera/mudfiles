...
lookupp(str) 
{
  ...
  if (str == "ALL") {
    ...
    array = sort_array(array,"alfa",this_object());
  ...
  if (database[str]) {
    write("Property: "+str+"\n");
    write("Type    : "+
	  sprintf("%-8@s\n",sort_array(database[str][1],"alfa",this_object())));
    write("Creator : "+database[str][0]+"\n");
...
print_out(namn,flag) {
  ...
  if (flags["V"]) 
    return sprintf("%s %20|s %s %-50=s\n",tmp,namn,database[namn][5]

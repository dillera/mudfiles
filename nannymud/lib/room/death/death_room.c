...
/* Deaths workroom?  Why not put the players in a less hospitable place? */
/* This is almost cosy! ;)   I'd have made it a black void or maybe the */
/* underground river of Styx that all dead people cross in the boat of */
/* Charon, the ferry man, in order to get to the land of the dead. */
/* (According to...uhmm...Greek mythology?  Well it's a nice concept!)  */
	dest_dir = 0;
...
add_player(plobj)
{
	...
	log_file("DEATH", ctime(time())+": "+capitalize(plobj->query_real_name()) +
		 " entered.\n");
...
remove_player(plobj)
{
	...
	log_file("DEATH", ctime(time())+": "+capitalize(plobj->query_real_name()) + " left.\n");
	if (i == 2)
	{
		if (players[0] == plobj)
		{
 ...
 * Description:   Write the list of players in room.
 */
write_list()
{
	...
	write("Total number of visitors: " + (nr / 2) + "\n");

	for (i = 0 ; i < nr ; i += 2)
		write("Visitor #" + (i + 1) + ": " + capitalize(players[i]->query_real_name()) + "\n");

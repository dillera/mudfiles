...
reset(arg) {
    ...
    set_light(1);
    short_desc = "Inside the Dark Temple";
    no_castle_flag = 1;
    long_desc = 
	"You are inside the Dark Temple,\n" +
	"where you can abolish your guardian angel\n" +
	"and enter the ranks of Those Who Kill Each Other.\n" +
    ...
    dest_dir = ({"room/dark_place", "west"});
    items = ({"altar","It's just an altar made of black stone"});
} /* reset */
...
throw_out_newbie()
{
  write("This place is much to scary for a young adventurer like you!\n");
  this_player()->run_away(); 
}

throw_out () {
    write ("\nThe smell of human blood lingers in the air; you fancy to hear the "
